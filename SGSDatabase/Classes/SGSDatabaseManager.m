...
@implementation SGSKeyValueItem
...
@interface SGSDatabaseManager ()
@property (nonatomic, strong, readwrite) FMDatabaseQueue * dbQueue;
@end

@implementation SGSDatabaseManager
...
- (void)createTableWithName:(NSString *)tableName {
    if ([SGSDatabaseManager checkTableName:tableName] == NO) {
        return;
    }
...
- (id)getObjectById:(NSString *)objectId fromTable:(NSString *)tableName {
    SGSKeyValueItem * item = [self getKeyValueItemById:objectId fromTable:tableName];
    if (item) {
...
- (SGSKeyValueItem *)getKeyValueItemById:(NSString *)objectId fromTable:(NSString *)tableName {
    if ([SGSDatabaseManager checkTableName:tableName] == NO) {
        return nil;
    }
    ...
    if (json) {
        ...
        SGSKeyValueItem * item = [[SGSKeyValueItem alloc] init];
...
- (NSArray *)getAllItemsFromTable:(NSString *)tableName {
    ...
    NSError * error;
    for (SGSKeyValueItem * item in result) {
        error = nil;
...
- (NSUInteger)getCountFromTable:(NSString *)tableName
{
    ...
    [_dbQueue inDatabase:^(FMDatabase *db) {
        FMResultSet * rs = [db executeQuery:sql];
        if ([rs next]) {
            num = (NSUInteger)[rs unsignedLongLongIntForColumn:@"num"];
        }
        [rs close];
    }];
    return num;
